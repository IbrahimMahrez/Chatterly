...
export function FeedbackProvider({ children }) {
  ...
  return <FeedbackContext.Provider value={{ showToast, confirmAction }}>
    {children}
    <div className="toast-stack" aria-live="polite">{toasts.map((toast) => <div className={`toast toast-${toast.type}`} key={toast.id}>{icon[toast.type]}<span>{toast.message}</span><button type="button" onClick={() => setToasts((current) => current.filter((item) => item.id !== toast.id))} aria-label="Close notification"><X size={16} /></button></div>)}</div>
    {confirmation && <div className="confirm-overlay" role="presentation"><section className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="confirm-title"><TriangleAlert className="confirm-icon" size={30} /><h2 id="confirm-title">{confirmation.title}</h2><p>{confirmation.message}</p><div className="confirm-actions"><button type="button" className="btn-secondary" onClick={() => closeConfirmation(false)}>إلغاء</button><button type="button" className="confirm-danger" onClick={() => closeConfirmation(true)}>{confirmation.confirmLabel}</button></div></section></div>}
